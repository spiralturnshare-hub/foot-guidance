...
## 2026-09-05(2): 撮影→アップロードの実機フィードバックを受けて3点修正

- 変更前 HEAD: `a35c69a`
- 冨永社長の実機テスト報告: 「撮影後『この画像を使う』を押すと別の画面(端末の共有シート)に遷移してしまい、upload-centerに自動で戻らない。足だけの写真では誰の撮影か分からないため、画像に注文者名も焼き込みたい。撮影者と実際に足を撮る人が別人のケースがあるため、撮影前に『◯◯様の撮影で間違いないか』の確認を挟みたい」。
- `src/pages/GuidancePage.tsx`:
  1. **埋め込みモード(upload-center/FF起動時)は端末保存(`navigator.share`のOS共有シート)を撤去**。以前は撮影→送信のたびに共有シートが挟まり、それが「別画面に遷移した」ように見えて混乱の原因になっていた。撮影→Green Storageへアップロード→自動で呼び出し元へpostMessage→`window.close()`、を直結。端末保存は非埋め込み(直リンク)時のみ従来どおり残す。
  2. **`uploads.insole_user_name`(発注時に登録された「何様の」インソールか)を取得し、画像の焼き込み(注文番号・撮影日時と並べて「お名前: ◯◯ 様」)に追加**。足だけの写真では誰の撮影か分からず、A4紙にお客様自身の手で名前を書いてもらっていた運用の代替。
  3. **新しい確認画面(`confirm-user`モード)を追加**: upload-center起動時、撮影ガイダンスに入る前に「これから撮影するのは◯◯様の足で間違いありませんか?」を表示し、明示的な確認を挟んでから撮影へ進む。注文者(操作している人)と実際に足を撮影される人が別人のケース(家族分の代理注文等)の取り違え防止。名前が取得できない場合(取得失敗・未登録)はこの画面をスキップして従来どおり撮影へ直行する(確認する対象が無いため)。
- ビルド OK(`npm run build`)。DB migration 不要(`uploads.insole_user_name`は既存列)。
- **未検証(実機)**: 3点とも実機での確認が必要。特に②の焼き込みが実際に撮影画像に反映されるか、③の確認画面が正しい名前を表示するか。
- 戻し方: `git revert` でこのコミットを打ち消す。

## 2026-09-05(3): 端末保存を「任意確認」に変更 + プラットフォーム別保存方式 + 単独アクセス時はお名前のみ

- 変更前 HEAD: `439b2c5`
- 冨永社長フィードバック: 「取扱店が事前撮影して後から端末の画像をアップロードするケースもあるので、埋め込みモードでも『端末に保存しますか?』と聞いてほしい。iOSの共有シートはボタンが多く分かりにくいので改善策を検討してほしい。単独アクセス(upload-center経由でない直接アクセス)もできるようにしてほしいが、その場合はオーダーIDでなく名前だけで十分」。
- `src/lib/saveImage.ts`:
  - **iOS Safariは「ワンタップ・無操作での自動保存」ができない仕様**(2026年時点も未解決。WebKit bug #167341)であることを調査確認。iOSは`navigator.share`(共有シート)以外に現実的な手段が無い。
  - **Android/デスクトップは`<a download>`が共有シートより手数が少なく無操作で保存できる**ため、iOSのみ`navigator.share`優先・それ以外は`<a download>`優先、に分岐(`isIOS()`判定を新設・export)。
- `src/pages/GuidancePage.tsx`:
  1. 埋め込みモード(upload-center/FF)でのGreenアップロード成功後、**即座に閉じず「端末にも保存しますか?」(はい/いいえ)を確認する新画面(ask-save)を追加**。「はい」時はクリックハンドラ内で直接`saveImageToDevice`を呼びnavigator.shareのuser activationを維持。iOSの場合のみ「次に共有画面が開きます。『画像を保存』を選んでください」の一言案内を追加表示。
  2. **単独アクセス(upload-center経由でないURLアクセス)時の入力フォームをお名前のみに簡略化**(従来はオーダーID必須)。オーダーIDは「どこにあるか分からない」お客様がほとんどのため。画像の焼き込みも、単独アクセス時の`userName`を`insole_user_name`の代わりに使うよう対応。
  3. 注文番号の焼き込み行は値がある時だけ表示(単独アクセス時は空欄のため非表示に)。
- **単独アクセスの使い方**: upload-centerを経由せず `https://foot-guidance.vercel.app/` を直接開く → お名前を入力 → 撮影 → 端末に保存(Greenへのアップロードは行われない、顧客情報が無いため)→ 後でupload-center Step2の「または手動で選択」から手動アップロード。
- ビルド・tsc OK。
- **未検証(実機)**: ask-save画面の表示・iOSでの共有シート案内・Android/デスクトップでの`<a download>`自動保存、いずれも実機確認が必要。
- 戻し方: `git revert` でこのコミットを打ち消す。

## 2026-09-05(4): 撮影画像の解像度が粗い問題 + 古い赤文字焼き込みの削除 + 焼き込み位置を上部へ + window.close()フォールバック

- 変更前 HEAD: `52e5727`
- 冨永社長の実機フィードバック: 「①保存後にupload-centerに自動で戻らない(画像は保存されている) ②画像左上に古い赤文字の顧客情報が別途あり、新しい白文字の情報は左下にあるが背景が白いと見えない ③ダウンロードされた画像が粗い。計測精度のため高解像度が必要」。
- **①原因**: `window.close()`はiOS Safari等でユーザー操作から時間が経つ・共有シート等の別UIを経由すると無視されることがある仕様(呼び出し側からは検知不能)。→ `src/pages/GuidancePage.tsx`: `window.close()`実行後、一定時間経っても画面が残っていれば「この画面を閉じてください」の案内を表示するフォールバックを追加。
- **②原因**: `src/components/Camera/CameraView.tsx`の`handleCapture`に、`annotateImage()`(GuidancePage側)とは別の**独立した古い赤文字焼き込みコード**が残っていた(`context.fillStyle="red"`で左上に`orderid`/`name`のURLパラメータを直接焼き込み。confirm-userで確認した`insole_user_name`等は反映されない別経路)。→ 削除。焼き込みは`annotateImage()`のみに一元化。あわせて`annotateImage.ts`の焼き込み位置を**下部→上部**に変更(下部には壁・かかと・A4用紙など計測に必要な内容が写ることが多いため)、文字に**黒縁取り(stroke)を追加**して背景色に関わらず視認できるようにした。
- **③原因**: `CameraView.tsx`の`handleCapture`が、撮影canvasを「カメラの実解像度」ではなく「画面上の表示サイズ(videoRect、CSSピクセル)× devicePixelRatio」で作成していた。実際のカメラ映像(1920x1080等)を画面サイズ相当まで縮小してから保存していたのが粗さの直接原因。→ canvasを`video.videoWidth`/`video.videoHeight`(カメラの実解像度)で作成し等倍描画するよう修正。あわせて`getUserMedia`の解像度要求を`1920x1080`→**`3840x2160`(4K ideal)**に引き上げ(端末が対応する最大解像度を使う。非対応の端末は自動で出せる範囲にフォールバックするため副作用なし)。中間フォーマットもPNG→JPEG(quality 0.95)に変更(この後annotateImage側で再度JPEG変換するため、PNG経由は4Kフレームで無駄にメモリを消費するだけだった)。
- **動画の解像度について(冨永社長より将来課題として明示)**: 現時点で foot-guidance に動画撮影機能は無い(写真のみ)。今後音声ガイダンス付きの動画撮影を追加する際は、同じ考え方(`getUserMedia`の`video`制約を端末の最大解像度に、`MediaRecorder`のビットレートも十分な値に)を適用すること。
- ビルド・tsc OK。
- **未検証(実機)**: 4点とも実機での確認が必要。特に解像度向上によりファイルサイズが増える(4Kは数MB〜十数MB想定。500MB上限設定済みのため問題ないはず)。
- 戻し方: `git revert` でこのコミットを打ち消す。

## 2026-09-05(5): 要求解像度を4K→フルHD(1920x1080)に修正

- 変更前 HEAD: `0b9b898`
- 冨永社長フィードバック: 「4Kは行き過ぎ。アップロード時間とStorageを圧迫する。普通の解像度でいい。低すぎるのも問題だが4Kも2Kも不要」。
- `src/components/Camera/CameraView.tsx`: `getUserMedia`の`width/height ideal`を`3840x2160`→**`1920x1080`(フルHD)**に戻す。ほぼ全てのスマホが標準対応しており、この用途(足の計測)の精度にはフルHDで十分。撮影canvasを実解像度で作成する修正(前回)自体は維持(画面表示サイズへの縮小は引き続き回避)。
- ビルド OK。
- 戻し方: `git revert` でこのコミットを打ち消す。
